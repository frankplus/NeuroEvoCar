...
public class Neat {
	...
	/**
	 * Amount a mutation_num changes on a link connecting a node that changed
	 * its trait
	 */
	...
	/**
	 * Probability that a link mutation which doesn't have to be recurrent will
	 * be made recurrent
	 */
	...
	public static void initbase() {
		...
		p_interspecies_mate_rate = 0.001;
	...
	public static boolean readParam(String xNomeFile, int readtype) {
		boolean ret = true;
		String xline;
		IOseq xFile;
		StringTokenizer st;
		...
		if(readtype==IOseq.TYPE_FILE)
			ret = xFile.IOseqOpenRfromFile();
		else if(readtype==IOseq.TYPE_RESOURCE)
			ret = xFile.IOseqOpenRfromResource();
