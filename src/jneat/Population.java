	...
	*/
	...
	public void epoch(int generation) {
		...
		while (itr_specie.hasNext()) {
			...
			rep1.append(" age since last improve ");
		...
		if (((Organism) curspecies.organisms.firstElement()).orig_fitness > highest_fitness) {
			...
			highest_last_changed = 0;
			// System.out.print("\n Good! Population has reached a new *RECORD
		...
		itr_specie = sorted_species.iterator();
		// System.out.print("\n verifica");
		// System.out.print("\n this species has "+sorted_species.size()+"
		// elements");
		while (itr_specie.hasNext()) {
			_specie = ((Species) itr_specie.next());
		...
		itr_specie = species.iterator();
		vdel = new Vector<Neat>(species.size());
		orgcount = 0;
		...
		itr_organism = organisms.iterator();
		vdel = new Vector<Neat>(organisms.size());

		while (itr_organism.hasNext()) {
			Organism _organism = ((Organism) itr_organism.next());
			if (_organism.pop_champ_child) {
				// System.out.print("\n At end of reproduction cycle, the child
				// of the pop champ is: ");
				break;
			}
		}
	...
	public Population(int size, int i, int o, int nmax, boolean r, double linkprob) {
		...
		highest_last_changed = 0;
		organisms = new Vector<Organism>(size);
		String fname_prefix = EnvRoutine.getUserFileData(EnvConstant.PREFIX_GENOME_RANDOM);

		for (count = 0; count < size; count++) {
			// new_genome = new Genome(count, i, o, NeatRoutine.randint(0,
			// nmax/8), nmax, r, linkprob);

			// modified by Francesco Pham
			new_genome = new Genome(count, i, o, 0, nmax, r, linkprob);
		...
		// System.out.print("\n The first innov_num available is
		// "+cur_innov_num);

		speciate();

		// backup of population
		fname_prefix = EnvRoutine.getUserFileData(EnvConstant.PREFIX_INIT_POPULATION);
			...
			try {
				...
				while (xline != "EOF") {
					...
					if (curword.equalsIgnoreCase("genomestart")) {
						...
						// System.out.print("\n");
						/*
						 * tmp2.append(" "+tmp1); System.out.print("\n tmp2 = "
						 * +tmp2);
						 */
