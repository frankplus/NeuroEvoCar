...
public class Genome extends Neat{
    ...
    public boolean verify() {
        ...
        // control if nodes in gene are defined and are the same nodes il nodes
        // list
        itr_gene = genes.iterator();
        while (itr_gene.hasNext()) {
            ...
            if (!nodes.contains(inode)) {
                System.out.println(
                        "Missing inode:  node defined in gene not found in Vector nodes of genome #" + genome_id);
                System.out.print("\n the inode is=" + inode.node_id);
                return false;
            }
            if (!nodes.contains(onode)) {
                System.out.println(
                        "Missing onode:  node defined in gene not found in Vector nodes of genome #" + genome_id);
        ...
        for (count = 1; count <= times; count++) {
            ...
            if (_gene.enable) {
                ...
                for (int j = 0; j < len_gene; j++) {
                    ...
                    if ((_gene.lnk.in_node == _jgene.lnk.in_node) && _jgene.enable
        ...
        if (found) {
            ...
            while (!done) {
                ...
                // OTHERWISE, match the innovation in the innovs list
                    ...
                    if ((_innov.innovation_type == NeatConstant.NEWLINK) && (_innov.node_in_id == thenode1.node_id)
                            && (_innov.node_out_id == thenode2.node_id) && (_innov.recur_flag == do_recur)) {
    ...
    public boolean mutate_add_node(Population pop) {
        ...
        while (!done) {
            ...
            // end for new innovation case
                ...
                if ((_innov.innovation_type == NeatConstant.NEWNODE) && (_innov.node_in_id == in_node.node_id)
                        && (_innov.node_out_id == out_node.node_id) && (_innov.old_innov_num == _gene.innovation_num)) {
                    // Create the new Genes
     ...
     * of hidden nodes nmax = number max of node this number must be greater or = (i + n +
    ...
    public Genome(int new_id, int i, int o, int n, int nmax, boolean r, double linkprob) {
        ...
        while (!done) {
            ...
            if (abort >= 20) {
                // if (abort == 10)
                // System.out.print("\n ALERT force new probability from 0.5 to
                // 1 step .01");
            ...
            for (col = 1; col <= totalnodes; col++) {
                for (row = 1; row <= totalnodes; row++) {
                    ...
                    if ((cmp[innov_number] && (col > i)) && ((col <= maxnode) || (col >= first_output))
