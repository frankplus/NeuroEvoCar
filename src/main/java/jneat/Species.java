...
public class Species extends Neat {
	 ...
	 */
	Vector<Organism> organisms = new Vector<Organism>(1, 0);
	...
	public boolean reproduce(int generation, Population pop, Vector<?> sorted_species) {
		...
		for (count = 0; count < expected_offspring; count++) {
			...
			if (thechamp.super_champ_offspring > 0) {
				...
				if ((thechamp.super_champ_offspring) == 1) {
					if (thechamp.pop_champ) {
						// System.out.print("\n The new org baby's (champion)
						// genome is : "+baby.genome.getGenome_id());
			...
			if (pop.species.isEmpty()) {
				pop.last_species++;
				newspecies = new Species(pop.last_species, true); // create a
																	// new
																	// specie
				pop.species.add(newspecies); // add this species to list of
												// species
				...
				// System.out.print("\n this is case of population with species
				// pree-existent");
				...
				if (!done) {
					pop.last_species++;
					newspecies = new Species(pop.last_species, true); // create
																		// a new
																		// specie
					pop.species.add(newspecies); // add this species to list of
													// species
