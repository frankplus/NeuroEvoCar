...
public class graph extends JPanel implements ActionListener {
	...
	public static void main(String[] args) {
		/*
		 * 
		 * JFrame jGrafo = null; graph sp = null;
		 * 
		 * try { jGrafo = new JFrame("graph subsystem");
		 * 
		 * sp = new graph(jGrafo); jGrafo.getContentPane().add(sp);
		 * 
		 * jGrafo.addWindowListener(new java.awt.event.WindowAdapter() { public
		 * void windowClosing(WindowEvent e) { System.exit(0); } });
		 * 
		 * jGrafo.pack(); jGrafo.setSize(500, 500); jGrafo.setVisible(true); }
		 * catch (Exception ex) { System.err.println("ERRORE"); }
		 * 
		 */
	...
	public void inizializza() {
		...
		p3.setBorder(BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder("grafo "),
				BorderFactory.createEmptyBorder(1, 1, 1, 1)));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals("Function")) {
			...
			for (int j = 0; j < 20; j++) {
				...
				double x2 = j * 2;
				v1.add(new code((int) x1, (int) y1, (int) x2, (int) y2, CodeConstant.LINE_TYPE_1, +1));
		...
		if (e.getActionCommand().equals("Cartesio")) {
			...
			// String nameGraph = "c:\\jneat\\dati\\genome.ex13"; // con lateral
			// connection sx-> dx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex14.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex20.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex.err3.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jes\\gui\\data\\genome.ex.err5"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex.err4.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex.err6.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex.err7.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex.err9.txt"; // con
			// lateral connection dx-> sx
			// String nameGraph = "c:\\jneat\\dati\\genome.ex.err10.txt"; // con
			// lateral connection dx-> sx
