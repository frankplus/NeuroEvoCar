...
public class mXY {
    ...
    /**
     * definisce la matrice di Vertex e inizializza tutti i Vertexer a 0 (prima
     * posizione disponibile sia su X che su Y
     */
    ...
    /**
     * visualizza il contenuto della matrice e i vari Vertexer
     */
    public void view() {
        Vertex _Vertex = null;
        System.out.print("\n ------------matrix mXY---------------");
    ...
    /**
     * step-1 : calcola massima larghezza in base a quanto calcolato da
     * expand_left; viene tenuto conto anche di eventuali spazi vuoti. step-2 :
     * punta alla riga max , la prima che incontra se + di una , a partire
     * dall'alto e la chiama Lc; quindi crea tre righe virtuali : La,Lb,Lc
     * copiando su La riga(0) copiando su Lb riga(1) copiando su Lc
     * riga(max)
     *
     * step-3 : punta alla prima riga La , alla seconda Lb; step-4 : scorre
     * La(i1) e per ogni elemento :
     *
     ...
     * con len less than 1 ; se esistono li prolunga in verticale ipotecando i posti
     * occupati e marcando i nodi
     */
    public void computeMacro(Vector<Edge> vEdge) {
        int minimo;
        ...
        /*
        * // calcola quanti nodi 'utili' ci sono nell'ultima riga // per
        * disporli // La = (dim_y - 1); tot_sensor = 0; for (i = 0; i < dim_x;
        * i++) { if (m[La][i] != null) tot_sensor++; }
        *
        * System.out.print("\n Layer = "+dim_y+" , -> numero di sensori = "
        * +tot_sensor); System.out.print("\n -----> numero di sensori = "
        * +tot_sensor);
        *
        * // in base al numero nodi e alla larghezza della matrice // calcola
        * il 'dx' tra i nodi // dx = 0; sp = 0; dx = (int) (dim_x /tot_sensor);
        * // ogni dx-unit piazzo un Va[i]; sp = (int) (dx / 2.0); //
        * spiazzamento iniziale pos = 0; for (iA = 0; iA < dim_x; iA++) { _o =
        * m[La][iA]; if (_o != null) {
        *
        * _o.barycentre = (int) (sp + pos * dx); _o.is_traversed = true; pos++;
        * matrix[La][_o.barycentre] = _o; System.out.print("\n piazzo Vertex="
        * + _o.name + " su colonna x= " + _o.barycentre); } }
        *
        *
        */
        ...
        for (iA = 0; iA < dim_x; iA++) {
            ...
            if (_o != null) {
                ...
                // System.out.print("\n piazzo Vertex=" + _o.name + " su colonna
                // x= " + _o.barycentre);
                ...
                if (_p_real != null) {
                    ...
                    // System.out.print("\n sotto tale nodo metto nodo " +
                    // matrix[Lb][_o.barycentre].name + " reale");
                    
                }
                
                //
                // se non esiste un nodo real e trova un nodo virtuale lo
                // posiziona sotto il nodo
                // e lo marca come già fatto.
                //
                    ...
                    // System.out.print("\n sotto tale nodo metto nodo "+
                    // matrix[Lb][_o.barycentre].name + " virtuale");
                }
        ...
        for (int layer = 0; layer < (dim_y - 1); layer++) {
            ...
            // System.out.print("\n ------------ INIZIO ANALISI LAYER : La=" +
            // La + " Lb=" + Lb + " ------------" );
            
            //
            // Questa seconda fase sistema per ogni nodo di out i restanti link
            // agendo a partire dalla coordinata del nodo di out e procedendo a
            // spirale
            ...
            // una volta ppiazzato il figlio , passa di nuovo al layer 0 e
            // piazza i restanti
            // nodi ;
            // Il motivo per cui non piazza subito tutti i nodi è che se si
            // agisse in tale senso
            // non sarebbe facile , poi , distribuire i nodi in modo uniforme
            // sotto i propri padri.
            //
            ...
            while (!fatto) {
                ...
                for (iA = 0; iA < dim_x; iA++) {
                    ...
                    // a questo punto una prima euristica potrebbe fare quanto
                    // segue :
                    // genera un vettore di nodi figli ordinato per distanza con
                    // solo i figli
                    // non ancora piazzati ; l'ordine prevede prima i figli con
                    // minore
                    // distanza e poi i figli con maggiore distanza ;
                    // in questo modo eventuali nodi reali che sono stati
                    // splittati fuori
                    // potrebbero tornare ad essere prioritari
                    // quindi la prima cosa da fare è :
                    // in base al nodo , alla lista dei figli, alla lista dei
                    // nodi successivi :
                    // per ogni nodo successivo che sia figlio e sia libero
                    // determina
                    ...
                    // una ulteriore euristica potrebbe riguardare il fatto di
                    // tracciare prima i nodi reali con il
                    // path minore dal layer in alto e via via , scendendo ,
                    // fissare gli altri nodi
                    // virtuali rispettando eventuali obblighi di stessa
                    // ascissa.
                    
                    // System.out.print("\n ..ciclo " + tenta + " con indice
                    // nodo su layer(" + La + ") = " + iA);
                    // view(matrix);
                    
                    if (_o != null) {
                        
                        // System.out.print("\n cerco link rimasti verso il nodo
                        // -> " + _o.name);
                        size_inp = _o.neighbors_inp.size();
                        ...
                        if (size_inp > 0) {
                            ...
                            // Cerca tra i figli di _o, il primo nodo
                            // discendente su layer successivo
                            // da piazzare
                            //
                            ...
                            for (iB = 0; iB < dim_x; iB++) {
                                _py = m[Lb][iB];
                                if ((_py != null) && (!_py.is_traversed) && (_o.neighbors_inp.contains(_py))
                                        && (_o.has_child(_py) || _py.has_child(_o))) {
                                    // System.out.print("\n trovato possibile
                                    // successore di "+_o.name+ " = "+
                                    // _py.name);
                            ...
                            for (iB = 0; (iB < dim_x) && (!done); iB++) {
                                ...
                                if (_py != null) {
                                    // System.out.print("\n analisi nodo " +
                                    // _py.name + " mark=" + _py.is_traversed);
                                    ...
                                    * System.out.print("\n    -> scarto " +
                                    * _py.name + " perchè non è figlio di " +
                                    * _o.name); else if (_py.is_traversed)
                                    * System.out.print("\n    -> scarto " +
                                    * _py.name + " perchè già fatto");
                                    */
                                }
                            ...
                            if (done) {
                                error = true;
                                // se trova un figlio da piazzare tra La e Lb ,
                                // cerca un intorno del baricentro del padre
                                // in cui mettere l'elemento;
                                // per fare questo procede a spirale con
                                // posizione di partenza = al baricentro del
                                // padre
                                // sommata alla funzione spirale;
                                // dato che per costruzione il numero di colonne
                                // = al massimo numero di nodi il posto
                                // deve essere trovato;
                                // se così non è allora esiste un errore nel
                                // calcolo o del baricentro o dell'assegnazione
                                // dei
                                // nodi
                                //
                                ...
                                for (int k = 0; k < max_range; k++) {
                                    ...
                                    if ((raggio >= 0) && (raggio < dim_x)) {
                                        ...
                                        // se raggio è nei limiti allora
                                        // verifico se il posto è disponibile
                                        //
                                        // System.out.print("\n okay indice " +
                                        // raggio+" corretto : verifico se
                                        // matrix["+raggio+"] è libero!");
                                        _pb = matrix[Lb][raggio];
                                        if (_pb == null) {
                                            // System.out.print("\n okay : posto
                                            // disponibile!");
                                            // System.out.print(" : posiziono
                                            // "+_py.name+" su
                                            // matrix["+Lb+"]["+raggio+"] ");
                            ...
                            if (error && _py != null) {
                                // System.out.print("\n *ERROR LOGIC * non
                                // trovato posto posto per nodo "+_py.name);
                            }
                ...
                if (_py != null) {
                    if (!_py.is_traversed) {
                        ...
                        for (int k = 0; k < max_range; k++) {
                            ...
                            // System.out.print("\n vedo se su
                            // matrix["+Lb+"]["+raggio + "] c'� posto!");
                            
                            // verifico che il raggio sia corretto
                            //
                            if ((raggio >= 0) && (raggio < dim_x)) {
                                
                                // se raggio � nei limiti allora verifico se il
                                // posto � disponibile
                                //
                                // System.out.print("\n okay indice " + raggio+"
                                // corretto : verifico se matrix["+raggio+"] �
                                // libero!");
                                _pb = matrix[Lb][raggio];
                                if (_pb == null) {
                                    // System.out.print("\n okay : posto
            ...
            //
            // A questo punto ipoteca i nodi virtuali estendendo le coordinate
            // verso il basso
            // In pratica cerca i nodi virtuali su Lb e se il nodo � una
            // estensione di un link
            // con len > 1 , lo propaga su matrix, con la stessa ascissa e
            // ordintata crescente
            ...
            for (iB = 0; iB < dim_x; iB++) {
                ...
                if (_o != null) {
                    ...
                    if (_o.is_virtual())
                        // if (_o.type == 4)
                    {
                        // okay nodo virtuale , vedo se f� parte un gruppo di
                        // nodi virtuali
                        // che rappresentano un lato di len > 1
                        //
                        // System.out.print("\n calcola figli virtuali di ->" +
                        // _o.name);
                        Vector<Vertex> Lista = new Vector<Vertex>(1, 0);
                        Vector<Vertex> l = GrafRoutine.L(_o, vEdge, Lista);
                        // System.out.print("\n provo a calcolare i figli..");
                        // System.out.print(" (lista ha len = " + l.size()+"
                        // )");
                        if (l.size() > 0) {
                            // dato che len > 1 , mi posiziono sul padre in Lb ,
                            // e calcolo
                            // il baricentro;
                            // quindi scorro la lista dei nodi virtuali e se ne
                            // esiste qualcuno
                            ...
                            for (i = 0; i < l.size(); i++) {
                                ...
                                // System.out.print("\n leggo elem(" + i + ") =
                                // " + e.name);
                                // se il nodo non � stato marcato lo imposta
                                // come marcato e
        ...
        *
        *
        * // a questo punto cerca il primo elemento utile in matrix // int
        * first_elem = 9999; for (j = 0; j < dim_y; j++) { // System.out.print(
        * "\n riga .."+j); for (i = 0; i < dim_x; i++) { // System.out.print(
        * "\n     colonna.."+i);
        *
        * if (matrix[j][i] != null) {
        *
        * if (first_elem > i) { if (i==0) { first_elem = 0; break; } else {
        * first_elem = i; break; } }
        *
        *
        *
        ...
        *
        * // quindi l'ultimo elemento utile // int last_elem = 0; for (j = 0; j
        * < dim_y ; j++) { // System.out.print("\n riga .."+j); for (i =
        * (dim_x-1); i > 0; i--) { // System.out.print("\n     colonna.."+i);
        * _o = matrix[j][i]; if (_o != null) { if (i == (dim_x-1)) { last_elem
        * = i; break; } else { if (last_elem < i) { last_elem = i; break; } } }
        * }
        *
        * if (last_elem == (dim_x-1)) break;
        *
        *
        * }
        *
     ...
     * posizione disponibile sia su X che su Y
     */
    public mXY(int row, int col, Vector<Vertex> _v) {
