...
public class Structure {
	...
	public Vector<Vertex> vVertex;

	// vettore contenente ref a oggetti di tipo connection
	public Vector<Edge> vEdge;
	...
	public Structure() {
		super();
		vVertex = new Vector<Vertex>(1, 0);
		vEdge = new Vector<Edge>(1, 0);
		vOut = new Vector<Vertex>(1, 0);
		vInp = new Vector<Vertex>(1, 0);
		vPlane = new Vector<planeXY>(1, 0);
		depth_y = 0;

	}

	/**
	 * crea in base alle connection definite le liste incoming - outgoing per
	 * ogni nodo
	 */

	public void genesis() {
		Iterator<Edge> itr_edge = null;
		Edge _edge = null;
		Vertex iVertex = null;
		Vertex oVertex = null;
		...
		while (itr_edge.hasNext()) {
			...
			if (_edge.active) {
				...
				if (iVertex != oVertex) {
					...
					// System.out.print("\n skip node "+iVertex.name+" cause
					// recursion");
				}
			}
		}

		Comparator<Vertex> cmp = new order_inner();
	...
	public void LoadGenome(String nomeFile) {
		...
		double weight = 0.0;
		hi_node_name = 0;
		Iterator<Vertex> itr_Vertex = null;
		boolean done;
	...
	/**
	 * Insert the method's description here. Creation date: (20/02/2002
	 * 11.56.44)
	 */
	...
	public void view() {
		Iterator<Vertex> itr_Vertex = null;
		Iterator<Edge> itr_edge = null;
		Iterator<planeXY> itr_plane = null;
		Edge _edge = null;
		Vertex _Vertex = null;
		planeXY _plane = null;
	...
	public void generate_virtual_Vertex_Edge_vertical() {
		...
		int diff_level = 0;
		int curr_edge_id = 0;
		Iterator<planeXY> itr_plane;
		planeXY _plane;

		boolean found;

		Vector<Edge> tmp = new Vector<Edge>(1, 0);
		...
		while (itr_edge.hasNext()) {
			...
			// " lev=" + _inode.level);
			// System.out.print(" , onode = " + _onode.name + " lev=" +
			// _onode.level);
			...
			if (diff_level == 0) {
				...
				// System.out.print("\n ** DISATTIVO ** link tra inode = " +
				// _inode.name + " lev=" + _inode.level);
				// System.out.print(" , onode = " + _onode.name + " lev=" +
				// _onode.level);
				_edge.active = false;
				...
				if (_inode.name == _onode.name) {
					...
					// System.out.print("\n creato recurrent node :
					// "+virtP.name+" per nodo "+_inode.name);
					vVertex.add(virtP);
					// _edge.active = true;
				}
...

			{
				...
				if (_inode.level > _onode.level) {
					...
					for (int livello = (to_node.level + 1); livello < from_node.level; livello++) {
						...
						// unica
						// il group id dei vertici è il nome del lato che li
						// connette
						...
						// System.out.print("\n added node virtual[" +
						// curr_edge_id + "] con level=" + virtP.level);
						...
						if (first) {
							...
							// il nome di gruppo dell'edge virtuale per i
							// vertuali nodi aggiunti
					...
					// il nome di gruppo dell'edge virtuale per i vertuali nodi
					// aggiunti
					// deve essere quello del lato che viene sostituito
					virtE.name = hi_edge_name++;
					virtE.id_gruppo = _edge.name;
					// System.out.print("\n added last new link from " +
					// from_node.name + " --> " + last_to_node.name);
					tmp.add(virtE);
					// System.out.print("\n okay");
				}
...

				{
					...
					virtE.id_gruppo = _edge.name;
					// System.out.print("\n added last new link from " +
					// last_to_node.name + " --> " + to_node.name);
					tmp.add(virtE);
					// System.out.print("\n okay");
	...
	public void generate_planeXY() {
		Vector<planeXY> tmp = new Vector<planeXY>(1, 0);
		int xdim;
		...
		// System.out.print("\n genera i piani XY in base alla matXY appena
		// generata");
		while (itr_plane.hasNext()) {
			_plane = ((planeXY) itr_plane.next());
			...
			if (_plane.vEdge.size() > 0) {
				...
				// System.out.print("\n Il piano di livello "+_plane.level +" ha
				// "+_plane.dim_y+" righe e avrà "+xdim+" colonne");
		...
		while (itr_plane.hasNext()) {
			...
			if (_plane.vEdge.size() > 0) {
				...
				while (itr_edge.hasNext()) {
					...
					// System.out.print("\n ho letto edge con inode = " +
					// _inode.name);
					// System.out.print(" , onode = " + _onode.name + " x=" +
					// _onode.gx);
					...
					if (diff_level > 1) {
						...
						if (_inode.gx < _onode.gx) {
							...
							for (int colonna = (from_node.gx + 1); colonna < to_node.gx; colonna++) {
								...
								// System.out.print("\n added node virtual
								// Horizontal[" + virtP.name + "] con level=" +
								// virtP.level);
								...
								if (first) {
									...
									// il nome di gruppo dell'edge virtuale per
									// i vertuali nodi aggiunti
									// deve essere quello del lato che viene
									// sostituito
									virtE.id_gruppo = _edge.name;
									first = false;
								}
	...
	/**
	 * ordina i nodi su vVertex per level battezza per ogni livello y, i nodi in
	 * numero crescente da 1 ad n dove n = numero di nodi per il corrente
	 * livello y
	 */
	...
	/**
	 * calcola il numero massimo di k-levels per determinare quante righe
	 * dovranno essere definite su mXY alla fine , quindi , è determinato
	 * depth_y = al numero REALE di livelli che quindi si estenderanno tra 0 e
	 * (depth_y-1)
	 */
	...
	public void compute_depth_y() {
		...
		Vertex _Vertey;
		Edge _Edge;
		Iterator<Vertex> itr_Vertex;
		Iterator<Vertex> itr_Vertey;
		Iterator<Edge> itr_Edge;
		 ...
		 * 
		 * System.out.print(
		 * "\n Dopo primo calcolo da inp  verso output  la situazione è : "
		 * +max_b); System.out.print(
		 * "\n ******************************************************"); //
		 * view(); System.out.print(
		 * "\n ******* compute.depth.y : max altitude_b =  "+max_b);
		 * System.out.print("\n ******* compute.depth.y : max altitude_f =  "
		 * +max_f);
		 * 
		 */
		...
		// a questo punto si esaminano i nodi dall'alto verso il basse e
		// viceversa
		// per ogni nodo non impostato :
		// se il predecessore non è settato cioè se non ha valori impostati cioe
		// level = -1 e altitude = -1
		// al predecessore viene assegnato il livello in funzione del suo
		// predecessore , si flagga
		...
		// gli eventuali nodi che restano a -1,-1 dovranno essere eliminati in
		// quanto non fanno
		...
		while (changed) {
			...
			while (itr_Vertex.hasNext()) {
				...
				// System.out.print(" che ha level = "+ _Vertex.level+" altitude
				// = "+_Vertex.altitude);

				// cerca il nodo più in basso verso cui il nodo è collegato
				// per fare questo esamino i nodi in uscita e vedo quale tra
				// questi ha il
				...
				if ((_Vertex.level == -1) && (_Vertex.altitude == -1)) {
					...
					while (itr_Vertey.hasNext()) {
						...
						// System.out.print("\n trovato vertice successore = " +
						// _Vertey.name);
						// System.out.print(" a level = "+ _Vertey.level+"
						// altitude = "+_Vertey.altitude);
					...
					if (found) {
						...
						// System.out.print("\n okay impostato " +
						// _Vertex.name);
						// System.out.print(" a level = "+ _Vertex.level+"
						// altitude = "+_Vertex.altitude);
					...
					// se non ci sono stati cambiamenti riprovo in senso opposto
						...
						// System.out.print("\n dato che non ho trovato alcun
						// successore provo con un predecessore ");
		...
		Vector<Vertex> vv = new Vector<Vertex>(1, 0);
		Vector<Edge> ve = new Vector<Edge>(1, 0);
		...
		while (itr_Vertex.hasNext()) {
			_Vertex = ((Vertex) itr_Vertex.next());
			if ((_Vertex.level == -1) && (_Vertex.altitude == -1)) {
				// System.out.print("\n ATTENZIONE : vertex isolato e messo in
				// lista per la cancellazione: "+_Vertex.name);
		...
		// per il momento non vengono visualizzati nodi che sono al di sotto dei
		// sensori anche perchè
		...
		// per fare questo si suppone che dopo quanto fatto sopra restino
		// proprio tali nodi
		// o al massimo i nodi isolati da tutti o reti interne isolate che per
		// tale applicazione al momento non sembrano essenziali.
		// (potrebbero cioè esistere delle sub-network internamente alla rete
		// funzionante;
		// tali sub-network potrebbero essere "l'inconscio" della rete e in
		// presenza di
		// crossing con altre reti alterare profondamente il compostamento della
		// rete)
		// procediamo ad eliminare sia i nodi senza coordinate , segnalandoli,
		// sia i link
		...
		// System.out.print("\n structure.compute.depth_y Elementi eliminati ->
		// "+nr_delete);
		...
		// System.out.print("\n structure.compute.depth_y Edge eliminati ->
		// "+nr_delete);
	 ...
	 * allora tale connessione è intrastrato in questo caso tutti i nodi di quel
	 * livello sono memorizzati nel piano
	 */
	public void create_plane() {
		...
		for (int l = 0; l < depth_y; l++) {
			...
			// in base alle connessioni genera un piano XY con tante righe
			// quante sono le connessioni
			// intra-strato , memorizzando anche i nodi per i quali dovrà essere
			// ricalcolato il livello
			// in quanto dovranmno fare posto alle connessioni intra-strato
			// per fare ciò legge i nodi del livello trovato e li memorizza su
			// un vettore;

			Vector<Vertex> vx = new Vector<Vertex>(1, 0);

			itr_Vertex = vVertex.iterator();
			while (itr_Vertex.hasNext()) {
				_p = ((Vertex) itr_Vertex.next());
				if ((_p != null) && (_p.level == l)) {
					// System.out.print("\n add to plane Vertex = "+_p.name+" ,
					// for level="+_p.level);
					vx.add(_p);
				}
			}

			// System.out.print("\n LAYER : "+ l +" nr.rows = " + count_aux_row
			// + " totals and " + depth_x + " column's");
	...
	public int generate_Grafo() {
		...
		// leggendo il vettore edge genera su ogni nodo le connessioni in
		// ingresso
		// e in uscita
		genesis();
		...
		// calcolati tutti i livelli calcola depth_x vale a dire il numero
		// massimo
		...
		// plainXY che rappresenta il singolo layer con i nodi del relativo
		// livello
		// in fase di creazione imposta il livello riallineandolo in base ai
		// livelli
		// aggiuntivi che saranno aggiunti in base alle connessioni
		// intra-strato;
		create_plane();
		...
		// genera i vertici virtuali e gli edge virtuali che sostituiscono
		// connessioni
		...
		// ricalcola depth_x vale a dire il numero massimo di nodi per un
		// singolo layer
			...
			for (int row = 0; row < depth_y; row++) {
				...
				for (int col = 0; col < depth_x; col++) {
					...
					if (_h_Vertex != null) {
						...
						// System.out.print(" x=" +_h_Vertex.x+" ,
						// y="+_h_Vertex.y);
	...
	/**
	 * scorre tutti i livelli a alla fine ritorna il numero massimo di nodi che
	 * ha trovato in un singolo livello
	 */
