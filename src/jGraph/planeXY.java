...
public class planeXY {
	...
	/**
	 * definisce la matrice di Vertex sul layer 'Lev'-esimo nell'asse x vengono
	 * disposti i vari punti mentre nell'asse y le estensioni per visualizzare
	 * le connessioni intrastrato; alla fine ho un rettangolo con dimensione
	 * pari a : righe = numero di connessioni intrastrato colonne = numero di
	 * Vertexs;
	 */
	public planeXY(Vector<Vertex> v, int _level, int row) {
		Vertex _p = null;
		Iterator<Vertex> itr_Vertex = null;
	...
	public int computeFreeLevel() {
		...
		for (int j = 0; j < dim_y * 2; j++) {
			...
			// System.out.print("\n verifico se libero axial("+j+") = "+axial+ "
			// (con offset="+offset_y);
