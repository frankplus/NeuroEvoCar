...
public class Vertex {
    ...
    public Vector<Vertex> neighbors_inp;
    
    // vector with outgoing node
    public Vector<Vertex> neighbors_out;
    
    // identificativo edge che collega questo Vertex utilizzato solo dai nodi
    // virtuali
    public int edge_id;
    ...
    public Vertex(int _x, int _y, int _t, int _id) {
        ...
        is_traversed = false;
        altitude = -1;
        neighbors_inp = new Vector<Vertex>(1, 0);
        neighbors_out = new Vector<Vertex>(1, 0);
        level = -1;
        Vertex_ref = null;
        edge_ref = null;
        zeta = -1;
    ...
    public int depth_back(int depth, int max_depth) {
        ...
        // System.out.print("\n BACK : --------- entro con depth "+depth+" per
        // input to node : "+name);
        //
        ...
        if ((neighbors_inp.size() == 0)) {
            // System.out.print("\n BACK : PSEUDO-SENSOR "+name+" has level
            // "+depth);
        ...
        Iterator<Vertex> itr_Vertex = neighbors_inp.iterator();
        ...
        while (itr_Vertex.hasNext()) {
            ...
            if (!(_Vertex.is_traversed)) {
                ...
                // System.out.print("\n BACK : curr dep return for
                // "+_Vertex.name+" = "+current_depth);
    ...
    public int depth_forw(int depth, int max_depth) {
        ...
        // System.out.print("\n FORW : --------- entro con depth "+depth+" per
        // input to node : "+name);
        //
        if (type == NeatConstant.OUTPUT) {
            // System.out.print("\n BACK : SENSOR "+name+" has level "+depth);
        ...
        // System.out.print("\n BACK : ...search incoming in level "+depth);
        // def iteratore per nodi di ingresso al presente
        Iterator<Vertex> itr_Vertex = neighbors_out.iterator();
    ...
    public boolean is_virtual() {
        ...
        if ((type == CodeConstant.VERTEX_BOT_Y) || (type == CodeConstant.VERTEX_MID_Y)
                || (type == CodeConstant.VERTEX_SNG_Y) || (type == CodeConstant.VERTEX_TOP_Y))
            return true;
    ...
    public boolean is_real() {
        ...
        if ((type == NeatConstant.SENSOR) || (type == NeatConstant.OUTPUT) || (type == NeatConstant.NEURON))
            return true;
